...
int trike_div(uint64_t high, uint64_t low, char **pp) {
  ...
  *pp = p+1;
  //printf("here: %s\n", p+1);
  assert(strcmp("6h7yzb4z112efh4n44afgst7a", buffer) == 0);
  return 0;
}
...
static PyObject* py_myfn2(PyObject* self, PyObject* args)
{
  ...
  while (PyLong_AsSsize_t(div) != 0L) {
    ...
    mod = PyTuple_GetItem(rem, 1);
    *(--p) = trans_table[PyLong_AsLong(mod)];
    //    Py_ssize_t v = PyLong_AsSsize_t(div);
  }
  //printf("yo %s\n", p);
  return PyUnicode_FromString(p);
  return obj;
...
static PyObject* py_myfn3(PyObject* self, PyObject* args)
{
  ...
  char *p = PyString_AS_STRING(str) + sz;
  *p = '\0';
  p -= 1;
...
static PyObject* py_myWowFunction(PyObject* self, PyObject* args)
{
  ...
  //  printf("5 %d\n", PyUnicode_Check(np));
  //  printf("5 %d\n", PyBytes_Check(np));
  char const *string_without_leaderC = PyBytes_AsString(np);
  //  printf("5x %p\n", string_without_leaderC);
  PyObject *np2a = PyBytes_FromString(&string_without_leaderC[3]);
  return np2a;
  // Remove the leader
  PyObject *np_ascii;
  np_ascii = PyUnicode_AsASCIIString(np);
  printf("7\n");
  char const *string_without_leaderA = PyBytes_AsString(np_ascii);
  printf("9x %p\n", string_without_leaderA);
  char const *string_without_leader = &PyBytes_AS_STRING(np_ascii)[3];
  printf("9y %p\n", string_without_leader);
  PyObject *np2;
  np2 = PyUnicode_FromString(&string_without_leader[3]);
  printf("B\n");
  return np2;
}
